    ...
    public class MediumRisk : ICategory
    {
        ...
        public string? Handle(Trade trade)
        {
            if (trade.Value > RiskValueConstant.RISK_CUTOFF_VALUE &&
                trade.ClientSector.Equals(ClientSectorConstant.PUBLIC_SECTOR.ToString(), StringComparison.CurrentCultureIgnoreCase))
            {
                return TradeCategoriesEnum.MediumRisk.ToString().ToUpper();
            }
